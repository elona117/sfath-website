...
const ApostolicGuide: React.FC<ApostolicGuideProps> = ({ onNavigate }) => {
  ...
  const formatText = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="font-black text-[#C9A24D]">{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const botText = response.text || "I apologize, the connection to the Vault is temporarily obscured. How else may I serve you?";
        ...
        <div className="w-[calc(100vw-2rem)] sm:w-[420px] h-[80vh] sm:h-[650px] flex flex-col overflow-hidden animate-reveal shadow-[0_40px_120px_rgba(0,0,0,0.4)] glass-dark border border-white/10 rounded-[2.5rem] sm:rounded-[3rem] origin-bottom-right">
          ...
          <div className="p-6 sm:p-8 pb-4 flex justify-between items-center border-b border-white/5 bg-white/5 backdrop-blur-md">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-white text-xs sm:text-sm font-cinzel font-black tracking-widest leading-none mb-1">Apostolic Guide</h4>
                <div className="flex items-center gap-2">
                  <span className="text-[7px] sm:text-[8px] text-[#C9A24D] uppercase tracking-[0.3em] font-black">Authorized Scribe</span>
                </div>
              </div>
            </div>
            <button 
          ...
          {/* Messages Area */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 no-scrollbar scroll-smooth bg-gradient-to-b from-transparent to-black/20">
            {messages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} animate-reveal`} style={{ animationDelay: '0.1s' }}>
                <div className={`flex flex-col gap-2 max-w-[90%] sm:max-w-[85%] ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                   ...
                   <div className={`px-5 py-4 sm:px-6 sm:py-4 text-[12px] sm:text-[13.5px] leading-[1.6] tracking-wide shadow-2xl ${
                    m.role === 'user' 
            ...
            {isTyping && (
              <div className="flex flex-col items-start gap-2">
                <span className="text-[7px] uppercase tracking-widest text-white/30 font-black px-1">Discernment in progress...</span>
          ...
          <div className="p-6 sm:p-8 pt-2 bg-gradient-to-t from-black/40 to-transparent">
            ...
            <div className="mt-5 flex gap-6 justify-center items-center">
               <button 
                onClick={() => { onNavigate('ADMISSIONS'); setIsOpen(false); }} 
               ...
               <button 
                onClick={() => { onNavigate('TRAINING'); setIsOpen(false); }} 
                className="text-[8px] sm:text-[9px] uppercase tracking-[0.4em] font-black text-[#C9A24D]/40 hover:text-[#C9A24D] transition-all hover:tracking-[0.5em] active:scale-90"
               >
                Systems
               </button>
               <span className="text-white/10 font-thin">|</span>
               <button 
                onClick={() => { onNavigate('RESOURCES'); setIsOpen(false); }} 
                className="text-[8px] sm:text-[9px] uppercase tracking-[0.4em] font-black text-[#C9A24D]/40 hover:text-[#C9A24D] transition-all hover:tracking-[0.5em] active:scale-90"
               >
                Vault
               </button>
        ...
        >
          ...
          <div className="relative flex flex-col items-center">
            <svg className="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span className="text-[5px] sm:text-[6px] uppercase tracking-widest font-black mt-1 opacity-60">Uplink</span>
          </div>
        </button>
      )}
    </div>
