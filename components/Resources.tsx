...
const Resources: React.FC = () => {
  ...
  const [isBriefingLoading, setIsBriefingLoading] = useState<string | null>(
    null
  );
  ...
  const resources: Resource[] = [
    {
      ...
      date: '2024-05-15',
      summary:
        'A deep dive into the primary functions of the modern apostle and the weights of governance.',
      fullText:
        'Apostolic mandate is defined not by title but by territory. In this teaching, we explore the four weights of governance: Spiritual Authority, Jurisdictional Oversight, Doctrinal Integrity, and Ministerial Succession. To carry an apostolic mandate is to become a master builder, aligning people with the divine pattern shown on the mount.',
    },
    {
      id: '2',
    ...
    },
    {
      ...
      date: '2024-04-10',
      summary:
        'Strategic guidance on maintaining apostolic order in a rapidly shifting global landscape.',
      fullText:
        'Volatility is the native environment of the apostle. To lead in such times requires an unshakeable inner governance. When systems fail, the apostolic structure must remain. This briefing covers the strategic implementation of internal peace, territorial mapping under pressure, and the maintenance of doctrinal clarity when the world is in chaos.',
    },
  ];
        ...
        contents: [
          {
            parts: [
              {
                text: `Say with institutional authority and spiritual weight: This is an SFATH Apostolic Briefing of "${res.title}" by ${res.author}. ${res.fullText || res.summary}`,
              },
            ],
          },
        ],
    ...
    <div className="max-w-7xl mx-auto px-6 py-24 bg-[#F9F9F7]">
      <div className="mb-24 flex flex-col md:flex-row justify-between items-end gap-12">
        <div className="max-w-2xl">
          ...
          <h1 className="text-5xl sm:text-7xl font-black text-[#0B1C2D] mb-8">
            The <span className="italic font-serif text-[#C9A24D]">Vault</span>
          </h1>
          <p className="text-stone-500 font-light text-xl leading-relaxed">
            Curated teachings, articles, and institutional downloads for
        ...
        <div className="flex flex-wrap gap-4">
          {['All', 'Teachings', 'Articles', 'Downloads'].map((cat) => (
            <button
              key={cat}
              className="px-8 py-3 text-[10px] uppercase tracking-widest border border-stone-200 hover:border-[#C9A24D] hover:text-[#C9A24D] transition-all rounded-full bg-white font-black"
            >
              {cat}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        {resources.map((res) => (
          <div
            key={res.id}
            className="group p-12 bg-white border border-stone-100 hover:border-[#C9A24D]/30 transition-all duration-500 shadow-xl hover:shadow-[#C9A24D]/5 rounded-3xl cursor-pointer flex flex-col"
          >
            <div className="flex justify-between items-center mb-10">
              <span
                className={`text-[9px] uppercase tracking-widest px-4 py-1.5 rounded-full font-black ${
                  res.category === 'Download'
                    ? 'bg-blue-50 text-blue-700'
                    : res.category === 'Teaching'
              ...
              <div className="flex items-center gap-4">
                    ...
                    {isBriefingLoading === res.id ? (
                      ...
                      <span className="flex items-center gap-2">
                        <span className="flex gap-0.5 h-2 items-end">
                          ...
                          <span
                            className="w-0.5 h-1/2 bg-emerald-500 animate-bounce"
                            style={{ animationDelay: '0.1s' }}
                          ></span>
                          <span
                            className="w-0.5 h-3/4 bg-emerald-500 animate-bounce"
                ...
                <span className="text-[10px] text-stone-300 font-black tracking-widest">
                  {res.date}
                </span>
              </div>
            </div>
            <h3 className="text-3xl font-cinzel font-black text-[#0B1C2D] mb-6 group-hover:text-[#C9A24D] transition-colors">
              {res.title}
            </h3>
            <p className="text-stone-400 text-[10px] uppercase tracking-[0.3em] font-black mb-4">
              By {res.author}
            </p>
      ...
      <div className="mt-32 p-16 bg-[#0B1C2D] rounded-3xl relative overflow-hidden text-center">
        ...
        <div className="relative z-10">
          <h3 className="text-3xl font-cinzel font-black text-white mb-6">
            Join the Mandate Briefing
          </h3>
          <p className="text-stone-400 text-lg mb-12 font-light max-w-xl mx-auto">
            Receive quarterly apostolic teachings and strategic campus alerts
