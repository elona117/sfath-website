          ...
          <div className="max-w-xl w-full bg-black border border-[#C9A24D]/40 rounded-3xl shadow-[0_0_50px_rgba(201,162,77,0.1)] overflow-hidden font-mono text-[9px] sm:text-xs">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="w-full bg-stone-900 h-2 rounded-full overflow-hidden border border-white/5">
                <div
                  className="h-full bg-[#C9A24D] transition-all duration-300 shadow-[0_0_20px_#C9A24D]"
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-12 gap-8">
          <div className="animate-reveal">
            <div className="flex items-center gap-4 mb-4">
              ...
              <button
                onClick={onLogout}
                className="text-[8px] uppercase tracking-widest font-black text-stone-400 hover:text-stone-900 transition-colors"
              >
                Close Admin Access
              </button>
            </div>
            <h1 className="text-4xl sm:text-6xl font-black text-[#0B1C2D] tracking-tight">
              Institutional{' '}
              <span className="italic font-serif text-[#C9A24D]">Registry</span>
            </h1>
          </div>
        ...
        {activeTab === 'System' && (
          <div className="bg-white rounded-[2rem] border border-stone-100 p-8 sm:p-16 shadow-2xl text-center max-w-2xl mx-auto animate-reveal">
            <h3 className="text-3xl font-cinzel font-black text-[#0B1C2D] mb-6">
              Gateway Governance
            </h3>
            <p className="text-stone-500 font-light mb-12 text-sm leading-relaxed">
              Suspending admissions will block new alignment requests.
            </p>
          ...
          <div className="bg-white rounded-[2rem] border border-stone-100 shadow-2xl overflow-hidden animate-reveal">
            <div className="p-8 border-b border-stone-50 flex flex-col sm:flex-row justify-between items-center gap-6">
              <div className="text-center sm:text-left">
                <h3 className="text-xs uppercase tracking-widest font-black text-[#0B1C2D]">
                  Waitlist Registry
                </h3>
                <p className="text-stone-400 text-[10px] italic">
                  {waitlist.length} souls pending notification
                </p>
              </div>
        ...
        {activeTab === 'Apps' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div
              className={`lg:col-span-4 bg-white rounded-[2rem] border border-stone-100 shadow-xl overflow-hidden flex flex-col h-[650px] ${selectedAppId ? 'hidden lg:flex' : 'flex'} animate-reveal`}
            >
              <div className="p-6 bg-stone-50 border-b border-stone-100 flex justify-between items-center">
                <div className="flex flex-col">
                  <h3 className="text-[10px] uppercase tracking-widest font-black text-[#0B1C2D]">
                    Registry
                  </h3>
                  <select
            ...
            <div
              className={`lg:col-span-8 bg-white rounded-[2rem] border border-stone-100 shadow-xl min-h-[650px] flex flex-col ${selectedAppId ? 'flex' : 'hidden lg:flex'} animate-reveal`}
            >
              {selectedApp ? (
                <div className="p-6 sm:p-12 flex flex-col h-full overflow-y-auto no-scrollbar">
                  <button
                    onClick={() => setSelectedAppId(null)}
                    className="lg:hidden mb-10 text-[9px] uppercase tracking-widest font-black text-stone-400 flex items-center gap-2"
                  >
                    Back to Registry
                  </button>
                  ...
                  <div className="space-y-10 flex-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <section>
                        <h5 className="text-[10px] uppercase tracking-[0.3em] font-black text-stone-400 mb-4">
                    ...
                    {selectedApp.communiqueHistory &&
                      selectedApp.communiqueHistory.length > 0 && (
                        <section className="pt-10 border-t border-stone-50">
                          <h5 className="text-[10px] uppercase tracking-[0.3em] font-black text-[#C9A24D] mb-6">
                            Communique History
                          </h5>
                          <div className="space-y-4">
                            {selectedApp.communiqueHistory.map((comm) => (
                              <div
                                key={comm.id}
                                className="bg-white p-6 rounded-2xl border border-stone-100 flex gap-6 group hover:shadow-lg transition-all"
                              >
                                ...
                                <div className="flex-1">
                                  <div className="flex justify-between items-center mb-1">
                                    <h6 className="text-[#0B1C2D] text-sm font-black">
                                      {comm.subject}
                                    </h6>
                    ...
                    <section className="pt-10 border-t border-stone-50 space-y-8">
                      <h5 className="text-[10px] uppercase tracking-widest font-black text-[#0B1C2D]">
                        Chancery Resolution
                      </h5>
                      <textarea
