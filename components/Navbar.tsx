    ...
    <>
      <div className="fixed top-0 left-0 right-0 z-[60] bg-[#0B1C2D] text-[7px] sm:text-[9px] uppercase tracking-[0.4em] text-[#C9A24D] h-6 sm:h-8 flex items-center overflow-hidden whitespace-nowrap border-b border-[#C9A24D]/20">
        <div className="animate-marquee flex gap-12 sm:gap-16 items-center">
          ...
          <span className="hidden sm:inline">• ADMISSIONS OPEN FOR NEXUS COHORT 2025</span>
          <span className="hidden sm:inline">• APOSTOLIC SYNOD GATHERING: OCTOBER 12TH</span>
        </div>
      </div>

      <nav className={`fixed top-6 sm:top-8 left-0 right-0 z-50 transition-all duration-700 px-3 sm:px-6 ${scrolled ? 'py-1 sm:py-2' : 'py-3 sm:py-6'}`}>
        <div className={`max-w-7xl mx-auto flex items-center justify-between transition-all duration-500 rounded-full px-5 sm:px-8 ${scrolled ? 'glass shadow-2xl border border-[#C9A24D]/10 h-14 sm:h-16' : 'bg-transparent border border-transparent h-16 sm:h-20'}`}>
          <div 
          ...
          <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {navLinks.map((link) => (
              ...
              >
                <span className={currentView === link.view ? 'text-[#C9A24D]' : 'text-[#0B1C2D]/60 group-hover:text-[#0B1C2D]'}>
                  {link.label}
                </span>
                <span className={`absolute bottom-0 left-0 h-[2px] bg-[#C9A24D] transition-all duration-500 rounded-full ${currentView === link.view ? 'w-full opacity-100' : 'w-0 opacity-0 group-hover:w-full group-hover:opacity-100'}`}></span>
              </button>
            ))}
          ...
          >
            <div className="w-6 h-4.5 flex flex-col justify-between items-end">
              <span className={`h-[2px] bg-current transition-all duration-500 ${isMenuOpen ? 'w-6 translate-y-2.5 rotate-45' : 'w-6'}`}></span>
              <span className={`h-[2px] bg-current transition-all duration-500 ${isMenuOpen ? 'opacity-0' : 'w-4'}`}></span>
              <span className={`h-[2px] bg-current transition-all duration-500 ${isMenuOpen ? 'w-6 -translate-y-1.5 -rotate-45' : 'w-5'}`}></span>
            </div>
          </button>
        </div>

        {/* Mobile Menu Overlay */}
        <div className={`fixed inset-0 bg-[#0B1C2D] transition-all duration-[800ms] cubic-bezier(0.16, 1, 0.3, 1) flex flex-col items-center justify-center space-y-8 z-[100] ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
          <button 
          ...
          <div className="pt-10 sm:pt-16 w-full px-10 sm:px-12 max-w-sm">
            ...
            <div className="mt-8 text-center">
              <p className="text-[#C9A24D]/40 text-[7px] sm:text-[8px] uppercase tracking-[0.5em] font-black italic">Apostolic Training Digital Hub</p>
            </div>
          </div>
        </div>
      </nav>
